    ...
    */
    ...
    public int write(byte[] data, int length, boolean wait)
    {
        ...
        if (data[0] == WRITE_COMMAND[0] && data[2] == WRITE_COMMAND[2]) {  // WRITE_COMMAND
            ...
            // Note: the buffer we were giving in this case is 208+5 bytes because the "WRITE_COMMAND" header is attached
            if (data[4] == (byte)208 ) {
                ...
                super.sendPacketToPC(data, 5, 208);
...
//                Log.v("Legacy", "WRITE: Delta Time = " + mDeltaWriteTime);
            ...
            if (data[4] == 3) { // Android asks for 3 bytes, initial query of device type
                ...
                queueUpForReadFromPhone(RECEIVE_SYNC_COMMAND_208);  // Send receive sync loop back
